...
var Senate = db.dbApi.define('senate', {
  ...
  title: {
    type: DataTypes.ENUM('senator','senate-majority-leader','senate-majority-whip','senate-minority-leader','senate-minority-whip'),
    allowNull: false,
    ...
    getAliases: function() {
      if (this.get('title') && this.get('first_name') && this.get('last_name') && this.get('party')) {
        ...
        if (title !== 'Senator') {
          ...
          aliases.push(title.replace('Senate ', '') + ' ' + name);
          aliases.push(title.replace('Senate ', '') + ' ' + last_name);
        }
